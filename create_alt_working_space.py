...
# Generate the alternate working space
alt_working_space = []

for theta1 in range(0, 361):  # 0 to 360 degrees
    for theta2 in range(0, 361):  # 0 to 360 degrees
        _, end_effector_pos = calculate_positions(theta1, theta2)
        alt_working_space.append({
            "theta1": theta1,
            "theta2": theta2,
...
# Write the alternate working space to a JSON file
with open('alt_working_space.json', 'w') as json_file:
    json.dump(alt_working_space, json_file, indent=4)

print("Alternate working space data has been written to alt_working_space.json") 